...
class ListaJogadoresView(discord.ui.View):
    ...
    def get_embed(self):
        ...
        embed.set_footer(text=f"VSO Guru · Página {self.pagina + 1}/{self.total_paginas} · {len(self.jogadores)} jogadores")
        return embed
...
class BotaoJogador(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=f"VSO Guru · {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
class BotoesObter(discord.ui.View):
    ...
    async def promover_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.jogador.get("imagem"):
            embed.set_thumbnail(url=self.jogador["imagem"])
        embed.set_footer(text="VSO Guru · Liga")
...
class SelectMembroView(discord.ui.View):
    ...
    async def select_membro(self, interaction: discord.Interaction, select: discord.ui.UserSelect):
        ...
        try:
            ...
            notif_embed.set_footer(text="VSO Guru · Liga")
...
class SelecionarAdversarioView(discord.ui.View):
    ...
    async def select_adversario(self, interaction: discord.Interaction, select: discord.ui.UserSelect):
        ...
        embed.set_footer(text=f"VSO Guru · Partida Amistosa · {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
class VerCartaSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=f"VSO Guru · Liga · {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
class ConfirmarDeleteView(discord.ui.View):
    ...
    async def confirmar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_footer(text=f"VSO Guru · ADM · {datetime.now().strftime('%d/%m/%Y %H:%M')}")
    ...
    async def cancelar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_footer(text="VSO Guru · ADM")
...
async def deletarplayer(interaction: discord.Interaction, nome: str):
    ...
    embed.set_footer(text=f"VSO Guru · ADM · {interaction.user.display_name} · {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
async def elenco(interaction: discord.Interaction, membro: discord.Member = None):
    ...
    embed.set_footer(text=f"VSO Guru · Liga · {datetime.now().strftime('%d/%m/%Y')}")
    await interaction.response.send_message(embed=embed)
